...
def infer_pairs(teams_df):
    """
    For demo/mocked data this pairs BOS-NYI, COL-TBL, BUF-UTA if present.
    With real adapters, you should map actual opponents from the schedule
    and pass a proper team->opponent dict here.
    """
...
def run_daily(cfg):
    ...
    # fetch bundle (mock or real adapter, depending on config)
    bundle = fetch_bundle(use_mock=cfg['data_sources']['use_mock'], games_date=games_date)
    ...
    # opponent map (for mock demo; swap to schedule-derived mapping with real adapter)
    ...
    pts_df = points_projection(
        player_star, toi_df, team_rates, goalies, opp_map,
        cfg['goalie']['beta_gsax'], cfg['pace']['use_geometric_mean']
    )
